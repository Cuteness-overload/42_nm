...
static int symbols_32(Elf32_Shdr *shs_table, uint16_t i, char *file_content, flags_t *flags, size_t file_size, const char * filename)
{
...
int ft_nm_32(char *file_content, struct stat file_stat, const char *filename, flags_t *flags)
{
	...
	for (uint16_t i = 0; i < e_shnum; i++) {
		...
		if (uint32_to_host(shs_table[i].sh_type, file_endian) == SHT_SYMTAB) {
			if ((ret = symbols_32(shs_table, i, file_content, flags, file_stat.st_size, filename))) {
				if (ret == 1)
					return ft_printf("ft_nm: %s: file format not recognized\n", filename);
				else
					return 1;
